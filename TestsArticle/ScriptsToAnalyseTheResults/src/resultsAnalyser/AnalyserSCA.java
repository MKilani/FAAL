...
public static void main(String[] args) throws Exception {
	...
	// Settings
		...
		String inputFileResultAnalysis = "2_SCA_results.txt"; //file with the results obtained by SCA - those obtained in the tests are provided in the folder: TestsArticle/Datasets/Results
		String benchmarkFile = "2_corpus_sca_benchmark.txt"; //benchmark file with the correct alignments for SCA - provided in the folder: TestsArticle/Datasets/Benchmarks
		String suffixMethod_Results = "SCA_2.txt"; //suffix indicating the method to use in the names of the output files (suggested: SCA + _ + nrCorpus)
	...
	String data2 = "";
		...
		for (int i = 1; i < lines.length; i = i+4){
		...
		for (int i = 1; i < resultLines.size(); i = i + 3){
			...
			//remove ";" in the comparison, so that matches in which SCA joins together two adjacent vowels are not penalised
		
			String alignment1 = resultLines.get(i).replaceAll(";", "");
			String benchmark1 = benchmark[i].replaceAll(";", "");
			
			String alignment2 = resultLines.get(i+1).replaceAll(";", "");
			String benchmark2 = benchmark[i+1].replaceAll(";", "");
			
			if(alignment1.equals(benchmark1) && alignment2.equals(benchmark2)){
