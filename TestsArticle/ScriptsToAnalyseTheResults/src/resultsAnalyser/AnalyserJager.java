...
public static void main(String[] args) throws Exception {
	...
	// Settings
		...
		String inputFileResultAnalysis = "2_jager_results.txt"; //file with the results obtained by jager - those obtained in the tests are provided in the folder: TestsArticle/Datasets/Results
		String benchmarkFile = "2_corpus_jager_benchmark.txt"; //benchmark file with the correct alignments for jager - provided in the folder: TestsArticle/Datasets/Benchmarks
		String suffixMethod_Results = "jager_2.txt"; //suffix indicating the method to use in the names of the output files (suggested: jager + _ + nrCorpus)
		...
		for (int i = 2; i < lines.length; i = i+7){
			
			
			String[] word1 = lines[i].split("(?!^)");
			String[] word2 = lines[i+2].split("(?!^)");
			...
			for (int n = 1; n < word1.length; n++){
				...
				lines[i] = lines[i] + ";" + word1[n];
				lines[i+2] = lines[i+2] + ";" + word2[n];
			}
			
			resultLines.add(lines[i-2]);
