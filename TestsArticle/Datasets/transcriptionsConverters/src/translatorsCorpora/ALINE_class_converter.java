...
public class ALINE_class_converter {
	...
	public static void main(String[] args) throws Exception {
		...
		//Location of the file encoding the correspondencies IPA - Aline - here in the classesFiles folder in the root of the project
		...
		//Name of the file encoding the correspondences IPA - Aline
		
		String nameClassesFile = "ALINE_classes";
				
		
			String sourceFile = "";
			String outputFile = "";
		
			//name of the faal input file to use as a model and of the Aline file as output - corpus to test
			if (option == 1) {
			sourceFile = "2_corpus_faal_input_no_alignments";
			outputFile = "2_corpus_aline_input_no_alignments";
			}
			
			//name of the faal input file to use as a model and of the aline file as output - benchmark
			
			if (option == 2) {
			sourceFile = "2_corpus_faal_benchmark";
			outputFile = "2_corpus_aline_benchmark";
			}
		...
		List<String> Class_ALINE = new ArrayList <>();
		List<String> Phone_ALINE = new ArrayList <>();
		...
		for (int n = 0; n < phones.length; n++){
			...
			Class_ALINE.add(items[0]);
			Phone_ALINE.add(phones[n]);
		}
		...
		for (int i = 1; i < lines_corpus.length; i = i +3){
			...
			for (int n = 0; n < word_A.length; n++){
				...
				for (int m = 0; m < Phone_ALINE.size(); m++){
				
					if (word_A[n].equals(Phone_ALINE.get(m))){
						word_A[n] = Class_ALINE.get(m);
					}	
				}	
			}
			...
			for (int n = 0; n < word_B.length; n++){
				...
				for (int m = 0; m < Phone_ALINE.size(); m++){
					...
					if (word_B[n].equals(Phone_ALINE.get(m))){
						word_B[n] = Class_ALINE.get(m);
					}	
				}
			}
